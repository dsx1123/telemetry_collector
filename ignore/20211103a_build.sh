...
function setup_influxdb() {
    ...
    if [[ $result == *'true'* ]]; then
        ...
        # So going forward, we need to create another auth which would consist of a user, org and bucket.
        # log "############################################"
        # log " create new telegraf data structure "
        # log "############################################"
        # docker exec -t influxdb influx org create -n telegraf
        # docker exec -t influxdb influx org find --json | jq -r ".[] | select(.name==\"telegraf\").id"
        # orgId=`docker exec -t influxdb influx org find --json | jq -r ".[] | select(.name==\"telegraf\").id"`
        # echo "orgId = $orgId"
        # docker exec -t influxdb influx org delete -i $orgId
        ...
        #       docker exec -t influxdb influx bucket create -n nxos_dialout -o Cisco -r 72h
        #       bucketId=`docker exec influxdb influx bucket list --json | jq -r ".[] | select(.name==\"nxos_dialout\").id"`
        #       echo "nxos_dialout bucketId id = $bucketId"
        #       docker exec -t influxdb influx auth create -o Cisco -d MDTBucketId --read-bucket d4b4ab7a5f374c93 --write-bucket d4b4ab7a5f374c93

        # echo "##################################################################################"
        # echo "##################################################################################"
        # echo " create telegraf user / pw: "
        # echo "###################################################"
        # docker exec -t influxdb influx user create -o $INFLUXDB_ORG -n $TELEGRAF_USER -p $GNMI_CERT_PASSWD
        # docker exec -t influxdb influx auth create -u $TELEGRAF_USER -d $TELEGRAF_USER --read-bucket $MDTBucketId --write-bucket $MDTBucketId
        # docker exec influxdb influx auth list --user $TELEGRAF_USER --json
        # echo "###################################################"
